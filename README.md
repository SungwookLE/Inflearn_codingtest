...
#### 5. 프로그래머스 문제: 등굣길 최단경로의 개수를 구하여라.

- 문제: [등굣길](https://programmers.co.kr/learn/courses/30/lessons/42898)
- 문제 설명
![](img/2022-03-28-20-29-49.png)
> 가장 왼쪽 위, 즉 집이 있는 곳의 좌표는 (1, 1)로 나타내고 가장 오른쪽 아래, 즉 학교가 있는 곳의 좌표는 (m, n)으로 나타냅니다.
> 격자의 크기 m, n과 물이 잠긴 지역의 좌표를 담은 2차원 배열 puddles이 매개변수로 주어집니다. 오른쪽과 아래쪽으로만 움직여 집에서 학교까지 갈 수 있는 최단경로의 개수를 1,000,000,007로 나눈 나머지를 return 하도록 solution 함수를 작성해주세요.

- 문제 풀이: BFS(너비우선탐색)처럼 최적의 최단 경로 한개를 구하는 문제가 아니고, 최단경로 모든 개수 합을 구하는 문제이다. BFS에서는 목표지점에 도착하면 함수를 종료하는 형태이므로, 해당 문제는 DP로 접근하여 풀자.
![](img/2022-03-28-20-37-24.png)
    ```c++
    if (주변에 물웅덩이가 없다면,)
        DP[i][j] = DP[i-1][j] + DP[i][j-1];
    ```
- 코드: [dp_programmers_go_school](./DynamicProgramming/dp_programmers_go_school.cpp)
